...
00:12 CUSTOMER: We are growing and onboarding is becoming a bottleneck — mostly handoffs between teams.
00:28 REP: Thanks for sharing that, Sarah — that makes sense. Can you describe where the handoff breaks down today?
...
01:44 REP: That helps. Our platform handles that end-to-end — we serve teams at your scale and our product integrates with your stack. Teams using a structured ramp and checklist integration have cut ramp by 40%. Would a pilot with your onboarding checklist make sense?
02:02 CUSTOMER: Yes, that aligns with our budget for this quarter.
02:14 REP: Great — I will send the pilot scope and success criteria by tomorrow. Does Thursday work to review?
...
00:10 CUSTOMER: Honestly, we are very unhappy. The last release caused an outage and our team lost data.
00:26 REP: I understand that is frustrating. Can we look at what happened?
00:36 CUSTOMER: We have had three incidents this month. We are considering cancellation if this is not resolved.
00:52 REP: I hear you. Let me get the right people involved.
01:02 CUSTOMER: We need a formal review and a corrective plan, or we will churn. This is unacceptable.
...
def build_history_seeds() -> dict[str, list[HistoryRecord]]:
    ...
    return {
        DEMO_REPS["coaching"]: [
            HistoryRecord(call_id="hist-daniel-1", decision="COACH", worst_dimension="discovery", worst_score=3.8, discovery_low=True),
            HistoryRecord(call_id="hist-daniel-2", decision="COACH", worst_dimension="discovery", worst_score=4.1, discovery_low=True),
            HistoryRecord(call_id="hist-daniel-3", decision="NO_ACTION", worst_dimension="rapport", worst_score=6.2),
            HistoryRecord(call_id="hist-daniel-4", decision="COACH", worst_dimension="discovery", worst_score=3.9, discovery_low=True, high_talk=True, rep_talk_ratio=0.79),
            HistoryRecord(call_id="hist-daniel-5", decision="COACH", worst_dimension="adaptability", worst_score=4.4, discovery_low=True),
        ],
        DEMO_REPS["healthy"]: [
            HistoryRecord(call_id="hist-sarah-1", decision="NO_ACTION", worst_dimension="rapport", worst_score=7.2),
            HistoryRecord(call_id="hist-sarah-2", decision="NO_ACTION", worst_dimension="discovery", worst_score=6.8),
            HistoryRecord(call_id="hist-sarah-3", decision="NO_ACTION", worst_dimension="adaptability", worst_score=7.0),
            HistoryRecord(call_id="hist-sarah-4", decision="NO_ACTION", worst_dimension="ecosystem", worst_score=6.5),
            HistoryRecord(call_id="hist-sarah-5", decision="NO_ACTION", worst_dimension="rapport", worst_score=7.1),
        ],
        DEMO_REPS["escalation"]: [
            HistoryRecord(call_id="hist-maya-1", decision="NO_ACTION", worst_dimension="rapport", worst_score=6.4),
            HistoryRecord(call_id="hist-maya-2", decision="COACH", worst_dimension="adaptability", worst_score=4.8),
        ],
    }
